...
def download_schedule(season, path_to_schedules=None):
    print("Scraping MBB schedules for year {}...".format(season))
    df = sdv.mbb.espn_mbb_calendar(season, ondays=True)
    calendar = df['dateURL'].tolist()
    ev = pd.DataFrame()
    for d in tqdm(calendar):
        date_schedule = sdv.mbb.espn_mbb_schedule(dates=d, groups=50)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_year', '-s', type=int, required=True, help='Start year of MBB Schedule period (YYYY), eg. 2023 for 2022-23 season')
    parser.add_argument('--end_year', '-e', type=int, help='End year of MBB Schedule period (YYYY), eg. 2023 for 2022-23 season')
